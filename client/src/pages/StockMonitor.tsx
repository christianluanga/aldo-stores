...
const StockMonitor: React.FC = () => {
  const { loading, error, shoes, updatedShoe } = useShoes({ source: 'STOCK' });
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timeId as NodeJS.Timeout);
  ...
  const paginatedShoes = filteredShoes.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
  ...
  const getRowClasses = (shoe: ShoeDTO, index: number, updatedShoe: ShoeDTO) => {
    const baseClass = index % 2 === 0 ? 'bg-blue-50' : 'bg-white';
    const updatedClass = shoe.id === updatedShoe?.id ? 'bg-green-100' : '';
    const lowInventoryClass = !isLowInStock && shoe.inventory! < LOW_STOCK ? 'bg-red-300 blink' : '';
    const hoverClass = !isLowInStock && shoe.inventory! < LOW_STOCK ? 'hover:text-white' : 'hover:bg-blue-100';
  ...
  return (
    <div className="container mx-auto my-8 px-4">
      {
        lowInStock.length >= 1 && (<Banner lowCountItems={lowInStock} handleClick={handleSeeLowInStock}/>)
      }
      ...
      <RequestModal
        ...
        modelShoes={shoes.filter(
          shoe => shoe?.model === selectedShoe?.model
        )}
        ...
        <table className="min-w-full bg-white border border-gray-200">
          ...
          <tbody>
            {paginatedShoes.map((shoe, index) => (
              ...
              >
                <td className="px-4 py-3 border-b text-center">{(currentPage - 1) * ITEMS_PER_PAGE + index + 1}</td>
                <td className="px-4 py-3 border-b text-center">{shoe.store?.name}</td>
                <td className="px-4 py-3 border-b text-center">{shoe.model}</td>
                <td className="px-4 py-3 border-b text-center">{shoe.inventory}</td>
      ...
      <div className="mt-4 flex justify-center items-center space-x-4">
        <button
          className={`px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-700 ${currentPage === 1 && 'opacity-50'}`}
        ...
        <button
          className={`px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-700 ${currentPage === totalPages && 'opacity-50'}`}
